...
#include <asm/div64.h>
#include "internal.h"
#include <../drivers/defrag/defrag_helper.h>
 ...
 */
static inline
struct page *buffered_rmqueue(int fp, struct zone *preferred_zone,
			struct zone *zone, unsigned int order,
	...
	for (o = 0; o < order; o++) {
		...
		if (!(alloc_flags & ALLOC_CMA)) {
			...
			if (free < 0)
				free = 0;
			free_pages -= free << o;
			if (!ctech_en) free_pages += z->free_area[o].nr_free_defrag << o;
		} else {
...
try_this_zone:
		page = buffered_rmqueue(fp, preferred_zone, zone, order,
		...
		unsigned long start_pfn, enum memmap_context context)
{
	...
	if (nid == 0) unmov_end_pfn = start_pfn + (size >> 2);
	for (pfn = start_pfn; pfn < end_pfn; pfn++) {
		/*
		...
		if ((z->zone_start_pfn <= pfn)
			 ...
			 * resulting from unmovable/reclaimable/movable interleaving
			 * allocation */
